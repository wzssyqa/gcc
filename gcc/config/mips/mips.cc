  ...
  switch (*symbol_type)
    ...
    case SYMBOL_PC_RELATIVE:
	 ...
	 but the original symbol-based access was known to be valid.  */
      if (TARGET_PCREL_PIC || GET_CODE (x) == LABEL_REF)
	return true;
			  ...
			  const char **relocs)
{
  ...
  if (TARGET_PCREL_PIC)
    {
      bool pcrel_hi = !strcmp (relocs[symbol_type], "%pcrel_hi(");
      bool pcrel_lo = !strcmp (relocs[symbol_type], "%pcrel_lo(");
      if (!pcrel_hi && !pcrel_lo)
	;
      else if (TARGET_OLDABI || pcrel_lo)
	fputs (" + 4", file);
      else if (!TARGET_PCREL_INSN && TARGET_NEWABI && pcrel_hi)
	fputs (" - 4", file);
    }
